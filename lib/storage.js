...
function Storage(opts) {
    ...
    this.modulesArchivePath = path.join(this.instancePath, 'cache.tar.gz');
...
/**
 * Создает на файловой систему структуру-заготовку для кэша с нужными правами
 */
Storage.prototype.prepareStorage = function() {
    // Создает структуру вида `%storageRoot%/%packageManager%`, дает группе права на запись туда
    try {
        fs.statSync(this.rootPath);
    }
    // Если statSync бросает исключение, значит, директории еще нет.
    catch(e) {
        // @see https://github.yandex-team.ru/an9eldust/packrat/issues/3
        this.runCommand('mkdir -p', this.rootPath);
        this.runCommand('chmod g+w', this.rootPath);
    }
};
...
Storage.prototype.exportToStorage = function(installProps) {
    ...
    this.runCommand('tar czf', this.modulesArchivePath, this.localPackagesDir);
    ...
    // @see https://github.yandex-team.ru/an9eldust/packrat/issues/3
    this.runCommand('chmod -R g+w', this.instancePath);
};
...
Storage.prototype.importFromStorage = function() {
    ...
    try {
        fs.statSync(this.modulesArchivePath);
        this.runCommand('tar xzf', this.modulesArchivePath);
    }
    // Если архива на месте нет, значит, кэш был создан предыдущей версией packrat,
    // которая вместо архива тупо копировала в кэш директорию as is
    catch(e) {
